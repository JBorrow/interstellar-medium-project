...
class Generator:
    ...
    def _gen_dm(self):
        ...
        self.dm_v_x, self.dm_v_y, self.dm_v_z = self._mod_v(self.dm_r)*self.nfw_gen.vel(self.dm_theta, self.dm_phi)
    ...
    def _gen_gas(self):
        ...
        self.gas_v_x, self.gas_v_y, self.gas_v_z = self._mod_v(self.gas_r)*gen_gas_r.vel(self.gas_r)

        return self.gas_theta, self.gas_z, self.gas_r, self.gas_v_x, self.gas_v_y, self.gas_v_z

    
    def _gen_velocities(self):
        self.dm_vel = self.gen_dm.vel(self.dm_r, self.dm_theta, self.dm_phi)
        self.gas_vel = self.gen_gas_r.vel(self.gas_r, self.gas_theta, np.zeros_like(self.gas_r))

        return self.dm_vel, self.gas_vel
...
if __name__ == "__main__":
    ...
    ax.quiver(gen.dm_x, gen.dm_y, gen.dm_z, gen.dm_v_x, gen.dm_v_y, gen.dm_v_z, length=10)
    ax.set_zlim(-400, 400)
